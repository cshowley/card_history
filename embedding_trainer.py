...
Architecture:
- Input: Card metadata (category, set, name, year, card_number) + grade
...
DB_NAME = "gemrate"
CARD_COLLECTION = "gemrate_pokemon_cards"
...
def build_weekly_sales_vectors(sales_df):
    """
    Build weekly sales "fingerprints" for each (card, grade) pair.
    ...
    Returns:
        keys: DataFrame with (universal_gemrate_id, grade, half_grade)
    ...
    keys = pd.DataFrame(
        price_wide.index.to_list(), columns=["universal_gemrate_id", "grade"]
    )
...
def normalize_grade(grade, half_grade):
...
class GradeProjector(nn.Module):
    """
    Projects grade (1-10) + half_grade flag into a dense embedding.
    """

    def __init__(self, input_dim=2, hidden_dim=32, out_dim=GRADE_EMBED_DIM):
        super().__init__()
...
class Encoder(nn.Module):
    """
    Main encoder network that maps (card_metadata, grade) → embedding.

    Architecture:
        metadata (1280d) + grade_embedding (64d) → 2048 → 1024 → 768
        Final output is L2-normalized.
    """

    def __init__(self, meta_dim, grade_dim=2, out_dim=ENCODER_OUT_DIM):
        super().__init__()
        self.grade_proj = GradeProjector(input_dim=grade_dim)
        self.fc_in_dim = meta_dim + GRADE_EMBED_DIM

        self.net = nn.Sequential(
            nn.Linear(self.fc_in_dim, ENCODER_HIDDEN_1),
            nn.ReLU(),
...
def compute_batch_loss(enc, xb_meta, xb_grade, sb, wb, tau):
    """
    ...
    Args:
        ...
        xb_grade: Batch of grade features
...
def train_embeddings():
    ...
    Steps:
        ...
        5. Generate embeddings for all (card, grade) combinations
    ...
    for i in range(len(sales_keys)):
        ...
        if gid in id_to_row:
            ...
            grade_feats.append(normalize_grade(g, h))
    ...
    for epoch in range(FINETUNE_EPOCHS):
        ...
        for batch_meta, batch_grade, batch_s, batch_w in train_loader:
            ...
            loss = compute_batch_loss(enc, b_meta, b_grade, b_s, b_w, FINETUNE_TAU)
    ...
    # Generate embeddings for all cards × grades
    print("Generating static card embeddings...")
    enc.eval()
    final_embs = []
    final_keys = []

    # Generate for all grade variants
    grade_half_pairs = [
        (g, h)

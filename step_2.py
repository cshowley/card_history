...
def s2_prepare_index_data(filepath):
    ...
    idx_df["index_ema_12"] = (
        idx_df["index_value"].ewm(span=constants.S2_INDEX_EMA_SHORT_SPAN).mean()
    )
    idx_df["index_ema_26"] = (
        idx_df["index_value"].ewm(span=constants.S2_INDEX_EMA_LONG_SPAN).mean()
    )
    return idx_df
...
def s2_load_and_clean_ebay(filepath):
    ...
    df = df[df["item_data.number_of_bids"] >= constants.S2_NUMBER_OF_BIDS_FILTER]
...
def s2_load_today():
    """Load today's data for inference - reads from catalog CSV."""
    print(f"Loading catalog from {constants.S2_INPUT_CATALOG_FILE}...")
    catalog_df = pd.read_csv(constants.S2_INPUT_CATALOG_FILE, low_memory=False)
    
    # Handle universal gemrate id
    ...
    gemrate_ids = catalog_df["GEMRATE_ID"].astype(str).tolist()
    
    all_today = []
    for grading_company in ["PSA", "CGC", "BGS"]:
        for grade in range(constants.S2_LOWEST_GRADE, constants.S2_HIGHEST_GRADE + 1):
...
def run_step_2():
    ...
    ebay_df = s2_load_and_clean_ebay(constants.S1_EBAY_MARKET_FILE)
    pwcc_df = s2_load_and_clean_pwcc(constants.S1_PWCC_MARKET_FILE)
    today = s2_load_today()
    ...
    df = df.loc[df["grade"] >= constants.S2_LOWEST_GRADE]
    df = df.loc[df["grade"] <= constants.S2_HIGHEST_GRADE]
    ...
    # Save features_prepped.csv for embedding training (historical sales only, no today)
    today_date = pd.to_datetime("today").normalize()
    hist_mask = df["price"].notna()
    features_prepped_df = df[hist_mask][["gemrate_id", "date", "price", "grade", "half_grade"]].copy()
    print(f"Saving {len(features_prepped_df)} rows to {constants.S2_FEATURES_PREPPED_FILE}...")
    features_prepped_df.to_csv(constants.S2_FEATURES_PREPPED_FILE, index=False)

    unique_ids = df["gemrate_id"].unique()
    batch_size = constants.S2_BATCH_SIZE

    index_df = s2_prepare_index_data(constants.S1_INDEX_FILE)
    ...
    for i in tqdm(range(0, len(unique_ids), batch_size), desc="Processing Batches"):
        ...
        batch_df, _ = s2_create_previous_sale_features(
            batch_df, constants.S2_N_SALES_BACK
        )
        batch_df, _ = s2_create_lookback_features(
            batch_df, constants.S2_WEEKS_BACK_LIST
        )
        batch_df, _ = s2_create_adjacent_grade_features(
            batch_df, constants.S2_N_SALES_BACK
        )
        ...
        if not df_hist.empty:
            ...
            if hist_writer is None:
                hist_writer = pq.ParquetWriter(
                    constants.S2_HISTORICAL_DATA_FILE, table_hist.schema
                )
        ...
        if not df_today.empty:
            ...
            if today_writer is None:
                today_writer = pq.ParquetWriter(
                    constants.S2_TODAY_DATA_FILE, table_today.schema
                )
    ...
    print(f"Saved historical data to {constants.S2_HISTORICAL_DATA_FILE}")
    print(f"Saved today's data to {constants.S2_TODAY_DATA_FILE}")

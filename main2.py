...
# Part 5: Hybrid Search Config
S5_N_NEIGHBORS_PREPARE = 100
S5_OUTPUT_NEIGHBORS_FILE = 'neighbors.parquet'

# Part 6: Neighbor Prices Config
S6_N_NEIGHBORS = 5
S6_OUTPUT_FILE = "features_prepped_with_neighbors.csv"
S6_ROLLING_WINDOW_DAYS = 30
...
def s2_create_lookback_features(df, weeks_back_list):
    ...
    weekly_agg = (
        df.groupby(["gemrate_id", "grade", "week_start"])[agg_cols]
        .mean()
        .reset_index()
    )
...
def s2_create_adjacent_grade_features(df, n_sales_back):
    ...
    for direction, grade_offset in [("above", 1), ("below", -1)]:
        ...
        merged = pd.merge_asof(
            df_lookup,
            ref_lookup[
                ["gemrate_id", "target_grade", "ref_date", "_sale_idx"]
            ],
            left_on="date",
            right_on="ref_date",
...
def run_step_2():
    ...
    catalog_df.loc[mask_univ, "GEMRATE_ID"] = catalog_df.loc[mask_univ, "UNIVERSAL_GEMRATE_ID"]
    ...
    print(f"  → Filtered {initial_count - len(df)} rows not in catalog. Remaining: {len(df)}")
...
# ==============================================================================
# PART 3: TEXT EMBEDDING FUNCTIONS
# ==============================================================================
...
def run_step_3():
    ...
    required_cols = ["GEMRATE_ID", "YEAR", "SET_NAME", "NAME", "CARD_NUMBER", "PARALLEL"]
...
# ==============================================================================
# PART 4: PRICE EMBEDDING FUNCTIONS
# ==============================================================================

class LSTMAutoencoder(nn.Module):
    def __init__(self, seq_len=S4_WINDOW_SIZE, n_features=1, embedding_dim=S4_PRICE_EMBEDDING_DIM):
...
def s4_train_and_extract(X_data, ids_map):
    ...
    extraction_loader = DataLoader(full_dataset, batch_size=S4_BATCH_SIZE * 2, shuffle=False)
...
def run_step_4():
    ...
    df_sales = pd.read_csv(S2_FEATURES_PREPPED_FILE, usecols=['gemrate_id', 'grade', 'date', 'price'], dtype={'gemrate_id': str})
...
# ==============================================================================
# PART 5: NEIGHBOR SEARCH FUNCTIONS
# ==============================================================================
...
def run_step_5():
    ...
    print(f"Loading price vectors from {S4_OUTPUT_PRICE_VECS_FILE}...")
    ...
    db_text_tensor, db_price_tensor, db_ids, all_text_df, all_price_df = s5_build_search_matrices(price_vecs, S3_OUTPUT_EMBEDDINGS_FILE)
    ...
    print(f"Saving final output with {len(results_df)} rows to {S5_OUTPUT_NEIGHBORS_FILE}...")
    results_df.to_parquet(S5_OUTPUT_NEIGHBORS_FILE)
    print("Step 5 Complete.")


# ==============================================================================
# PART 6: NEIGHBOR FEATURES FUNCTIONS
# ==============================================================================

def s6_prepare_price_lookup(df):
    """
    Creates a dense lookup table for (gemrate_id, grade) -> daily moving average price.
    """
    print("Preparing neighbor price lookup...")
    df = df.sort_values(['gemrate_id', 'grade', 'date'])
    subset = df[['gemrate_id', 'grade', 'date', 'price']].copy()
    pivot = subset.pivot_table(index='date', columns=['gemrate_id', 'grade'], values='price', aggfunc='mean')
    pivot_daily = pivot.resample('D').asfreq()
    rolling_daily = pivot_daily.rolling(window=f'{S6_ROLLING_WINDOW_DAYS}D', min_periods=1).mean()
    lookup_df = rolling_daily.stack(['gemrate_id', 'grade']).reset_index()
    lookup_df.rename(columns={0: 'rolling_avg_price'}, inplace=True)
    return lookup_df
...
def run_step_6():
    ...
    if not os.path.exists(S2_FEATURES_PREPPED_FILE):
        print(f"Error: {S2_FEATURES_PREPPED_FILE} not found.")
        return
    if not os.path.exists(S5_OUTPUT_NEIGHBORS_FILE):
        print(f"Error: {S5_OUTPUT_NEIGHBORS_FILE} not found.")
        return
    ...
    lookup_df = s6_prepare_price_lookup(df_sales)
    
    print("Processing neighbor sales...")
    df_neighbors['rank'] = df_neighbors.groupby('gemrate_id').cumcount() + 1
    pivot_neighbors = df_neighbors.pivot(index='gemrate_id', columns='rank', values=['neighbors', 'score'])
    pivot_neighbors.columns = [f"{c[0]}_{c[1]}" for c in pivot_neighbors.columns]
    pivot_neighbors = pivot_neighbors.reset_index()

    df_sales = df_sales.reset_index(drop=True)
    df_sales['_row_id'] = df_sales.index
    base_sales = df_sales[['_row_id', 'gemrate_id', 'grade', 'date']].copy()
    
    merged = base_sales.merge(df_neighbors, on='gemrate_id', how='left')
    merged.rename(columns={'neighbors': 'neighbor_id'}, inplace=True)
    
    print("  Looking up neighbor prices step 1 of 2...")
    merged = merged.sort_values('date')
    lookup_df = lookup_df.sort_values('date')
    feature_rows = pd.merge_asof(
        merged,
        lookup_df,
        left_on='date',
        right_on='date',
        left_by=['neighbor_id', 'grade'],
        right_by=['gemrate_id', 'grade'],
        direction='backward'
    )
    feature_rows = feature_rows.dropna(subset=['rolling_avg_price'])
    feature_rows = feature_rows.sort_values(by=['_row_id', 'rank'])
    feature_rows['new_rank'] = feature_rows.groupby('_row_id').cumcount() + 1
    feature_rows = feature_rows[feature_rows['new_rank'] <= S6_N_NEIGHBORS]

    wide_features = feature_rows.pivot(
        index='_row_id', 
        columns='new_rank', 
        values=['score', 'rolling_avg_price']
    )

    new_cols = []
    for r in tqdm(range(1, S6_N_NEIGHBORS + 1)):
        col_name_score = f"neighbor_{r}_score"
        if ('score', r) in wide_features.columns:
            wide_features.rename(columns={('score', r): col_name_score}, inplace=True)
        else:
            wide_features[col_name_score] = np.nan
        new_cols.append(col_name_score)
        col_name_price = f"neighbor_{r}_avg_price"
        if ('rolling_avg_price', r) in wide_features.columns:
            wide_features.rename(columns={('rolling_avg_price', r): col_name_price}, inplace=True)
        else:
            wide_features[col_name_price] = np.nan
        new_cols.append(col_name_price)
    print("  Looking up neighbor prices step 2 of 2...")
    flattened_data = {}
    for r in tqdm(range(1, S6_N_NEIGHBORS + 1)):
        if ('score', r) in wide_features.columns:
            flattened_data[f"neighbor_{r}_score"] = wide_features[('score', r)]
        else:
            flattened_data[f"neighbor_{r}_score"] = np.nan
            
        if ('rolling_avg_price', r) in wide_features.columns:
            flattened_data[f"neighbor_{r}_avg_price"] = wide_features[('rolling_avg_price', r)]
        else:
            flattened_data[f"neighbor_{r}_avg_price"] = np.nan
            
    wide_features_flat = pd.DataFrame(flattened_data, index=wide_features.index)
    wide_features_flat = wide_features_flat.reset_index()

    final_df = df_sales.merge(wide_features_flat, on='_row_id', how='left')
    final_df.drop(columns=['_row_id'], inplace=True)
    final_df = final_df.dropna(subset=['prev_1_price', 'neighbor_1_score'], how='all')

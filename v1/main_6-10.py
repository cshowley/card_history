    ...
    for card_id, card_group in tqdm(df.groupby('card_id'), desc="Processing cards"):
        ...
        for grade, history in grade_histories.items():
            ...
            for i in range(1, len(prices)):
                ...
                all_prices = np.zeros((6, max_seq_len))  # 6 grades instead of 11
                all_gaps = np.zeros((6, max_seq_len))
                all_lengths = np.zeros(6, dtype=int)                
                ...
                for idx, (g, prices_list, gaps_list, length) in enumerate(zip(
                    ...
                    # Map grades 5-10 to indices 0-5
                    adjusted_g = g - 5
                    all_lengths[adjusted_g] = length
                    all_prices[adjusted_g, :length] = prices_list
                    all_gaps[adjusted_g, :length] = gaps_list
                
                # Store training example
                all_data.append({
                    'card_idx': card_idx,
                    'target_grade': float(grade - 5),  # Map PSA grades 5-10 to indices 0-5                    'all_prices': all_prices,
                    'all_gaps': all_gaps,
...
def calculate_full_grade_loss(predictions, targets, all_lengths, all_prices, model,
    ...
    for i in range(batch_size):
        ...
        for grade in range(5, 11):  # Start from grade 1 since we compare with grade-1
...
def format_full_grade_inference(
    ...
    grade_histories = {g: [] for g in range(5, 11)}
    ...
    for raw_grade in range(5, 11):
        grade = raw_grade - 5  # Map to 0-5 index
        history = grade_histories[raw_grade]
        n_sales = len(history)
        
        if n_sales > 0:
            # Store prices using the mapped index
...
def main(args):
    ...
    model = FullGradeHistoryPredictor(
        ...
        # n_grades=11 #when using all grades
        n_grades=6 #when using grades 5-10
    )

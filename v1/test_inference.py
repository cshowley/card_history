...
def predict_price(model, card_id, target_grade, sales_records, card_to_idx, max_seq_len=5, device='cpu'):
    ...
    if inference_input['all_lengths'][0][target_grade] == 1:
...
def format_full_grade_inference(
    ...
    # CRITICAL FIX: Initialize all lengths to 1 (not 0)
    all_lengths = np.ones(n_grades, dtype=int)  # Changed from np.zeros
...
raw_df = pd.read_csv('full_training_data.csv')
card_ids = [3700060, 544027, 9656727, 7869313, 9603030]
card_id = card_ids[4]
grade = [i for i in range(0,11)]
...
for target_grade in grade:
    ...
    print(f'Grade {target_grade} prediction: {prediction}')
print(card_id)
...
output_csv_name = 'full_training_data_auth.csv'
...
for i in tqdm(range(0, df_spec_id.shape[0], chunk_size)):
	...
	pipeline = [
		{"$match": {"item_data.format": "auction"}},
		# {"$match": {'market_transaction.auctionHouse': 'eBay'}},
		# {"$match": {'market_transaction.attributes.gradingCompany': 'PSA'}},
		{"$match": {"gemrate_data.specid": {"$in": spec_id_chunk}}},
		# {"$match": {"_id": {"$in": [ObjectId(tmp) for tmp in id_list]}}},
		{"$project": {
			"gemrate_data.grader": 1,
			# "item_data.item_specifics.Grade": 1,
			# "grade": 1,
	...
	df['item_data.date'] = pd.to_datetime(df['item_data.date'])
	# pattern_price = r'\$([^$]*)'
	# df['item_data.price'] = df['item_data.price'].apply(lambda x: re.search(pattern, text))
	# pattern_currency = r'([^$]+)\$'
	df['item_data.price_currency'] = df['item_data.price'].str.extract(r'([^$]+)\$')[0]
	df['item_data.price'] = df['item_data.price'].str.extract(r'\$([^$]*)')[0]
	df['item_data.price'] = pd.to_numeric(df['item_data.price'].str.replace(',', ''), errors='coerce')
	df['data_type'] = 'new'
	...
	df["grade_processed"] = pd.to_numeric(df.gradeNumber.apply(lambda x: x.replace('g','').replace('_','.')), errors='coerce')
